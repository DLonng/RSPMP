...
SemanticsBayesian SemanticsBayesian::semanticFusion(const SemanticsBayesian s1, const SemanticsBayesian s2)
{
    ...
    for (int i = 0; i < semantic_colors.size(); i++) {
        ...
        //semantic_colors[i].confidence = semantic_colors[i].confidence * confidences2[i];
        
        // 1 : k - 1 时刻当前节点语义概率分布
        //float p_1_to_k_1 = semantic_colors[i].confidence;
        // k 时刻当前节点语义概率分布
        //float p_k = confidences2[i];
        // 1 : k 时刻当前节点 Bayes 修正后的语义概率分布
        //float p_1_to_k = p_1_to_k_1 * p_k;
